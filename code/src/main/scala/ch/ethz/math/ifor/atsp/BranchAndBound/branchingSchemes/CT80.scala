...
object CT80 extends BranchingScheme {
  ...
  def listChildren(branchNode: BranchNode):List[BranchNode]={
    ...
    if (branchNode.isRootNode){
      val reducedThreshold = branchNode.globalUpperbound - branchNode.lowerBound
...
//TODO: use iterableObject.map instead of for (thing <- iterableObject) when possible
    ...
    for (i <- listArcs.indices){
      ...
      */
      ...
      var newNode = new BranchNode(branchNode.input, finalMap,branchNode.useAdditive,false,Map(listArcs(i)._1->listArcs(i)._2))
