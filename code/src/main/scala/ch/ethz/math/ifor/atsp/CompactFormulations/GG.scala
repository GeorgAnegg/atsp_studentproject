...
object GG extends Solver {
  ...
  def solve(input: Input,formulation:String,preprocessing:Boolean,useAdditive:Boolean,useParametricAP:Boolean,useConnecting:Boolean): Output={
    ...
    def constructVariableForG(site1:Site ,site2:Site):MPVariable= {
      if (site1.id==site2.id) {solver.makeNumVar(0,0,"")}
      else {
        solver.makeNumVar(0,inf,"")
      }
    }

    val x: arcWise[MPVariable] = arcWise(input, constructVariable)

    // construct g variables
    val g: arcWise[MPVariable] = arcWise(input, constructVariableForG)

    // constraints for g
    for (i <- input.sites){
      if (i != input.sites.head){
        val constraint = solver.makeConstraint(1, 1, "")
        for (j <- input.sites){
          constraint.setCoefficient(g.search(i,j), 1)
          if (j != input.sites(1)) {
            constraint.setCoefficient(g.search(j,i), -1)
          }
        }
      }
    }
    ...
    for (i <- input.sites){
      if (i!=input.sites.head){
        for (j <-input.sites){
          val constraint = solver.makeConstraint(-inf, 0, "")
          constraint.setCoefficient(g.search(i,j), -1)
          constraint.setCoefficient(x.search(i,j), 1-input.sites.length)
        }
      }
    }
