...
import ch.ethz.math.ifor.atsp._
...
object EdmondsTest {
  def compute(input: Input, branchNode: BranchNode): Double = {
    ...
    println("rootnode is: " + rootSite)

    // construct cost graph
    val costs: arcWise[Double] = arcWise(branchNode.input, input.distance)
    // block ii entries

    var costsPrime : Map[Site, Map[Site, Double]] = costs.entries.map {
      case (site1, map1) => (site1, map1.map {
        case (site2, _) if branchNode.varAssignment(site1)(site2)==Some(false) => (site2, inf)
        case (site2, _) if branchNode.varAssignment(site1)(site2)==Some(true) => (site2, 0.0)
    ...
    def chuLiuEdmonds(graph: Map[Site, Map[Site, Double]]): Map[Site, Map[Site, Boolean]] = {
      ...
      if (firstCycleFound.isEmpty) {
        println("no cycle, find min arbre")
    ...
    def inDegreeSites(graph: Map[Site, Map[Site, Double]], site: Site): (List[(Site, Site)], Map[Site, Double]) = {
      ...
      //println("list in-site")
      //result.foreach{e=>println(site,e._1,e._2,graph(e._1)(site))}
      ...
      //println("site: "+site, "in site: ",minInSite)
