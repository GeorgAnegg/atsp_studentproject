...
object HungarianTest extends LowerBoundSolver{
  def compute(branchNode: BranchNode): (Map[Site, Map[Site, Boolean]],Map[Site, Map[Site, Double]]) = {
    ...
    /*
    var sitesRight: Array[Site] = {
      sitesLeft.map { site => new Site(site.id + "Right") }
    }
     */
    var sitesRight: Array[Site] = Array()
    var lrMap:Map[Site,Site]=Map()
    for(e <- sitesLeft){
      val newRight = new Site(e.id+"Right")
      sitesRight = sitesRight :+ newRight
      lrMap = lrMap ++ Map(e->newRight)
    }
    ...
    var reducedCost = stMap.collect{
      case (site1, map1) => (site1, map1.collect{
        case (site2, value) if !site1.id.contains("Right") && site2.id!="s" => (site2,value)
      })
    }
    finalMatching.foreach{
      case (i,j) => reducedCost = reducedCost.updated(i,reducedCost(i)++Map(j->0.0))
    }
    reducedCost = reducedCost.collect{
      case (site1, map1) => (site1, map1.collect{
        case (site2, value) if site2.id.contains("Right") => (lrMap.find(_._2.id==site2.id).get._1,value)
    ...
    def constructResult(site1:Site ,site2:Site):Boolean= {
      if (finalMatching(site1)==site2) {
        costFinal += branchNode.costsMap(site1)(site2)
        true}
      else {false}
    }
