...
JAVA_OPTS="-Djava.library.path=$OR_TOOLS_HOME/lib"

for instance in $INSTANCES; do
	for solver in $SOLVERS; do
		timeout --foreground 3 $SCALA_INTERPRETER -cp $ASSEMBLY:$SCALA_RUNTIME_LIBRARY instAlgoIncremental.scala $instance $solver
	done
done
